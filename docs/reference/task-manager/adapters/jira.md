...
O Google Antigravity também oferece um **MCP server oficial da Atlassian** que pode substituir o `fetch` direto. Para usar:

1. Configurar MCP em `~/.gemini/config/mcp_config.json` (via `...` → MCP Servers → View raw config)
2. Autenticar: ferramentas `mcp__Atlassian__authenticate` e `mcp__Atlassian__complete_authentication`
