...
**✅ FORMATO CORRETO:**
```bash
# Comando para mover sessões versionadas
mv docs/sessions/nome-sessao docs/sessions/archived/$(date +%Y-%m-%d_%H%M)_nome-sessao
...
grep -r "Y-%m-%d %H:%M" .agents/workflows/
grep -r "toLocaleString()" docs/reference/

# Buscar padrões corretos
grep -r "%d/%m/%Y %H:%M" .agents/workflows/
```
...
- ✅ `.agents/workflows/git-feature-start.md`
- ✅ `.agents/workflows/engineer-hotfix.md`
- ✅ `.agents/workflows/git-hotfix-start.md`

### **Documentos Já Corretos:**
- ✅ `.agents/workflows/git-README.md` (linha 464)
