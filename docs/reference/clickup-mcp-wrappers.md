...
Centralizar todas as chamadas MCP do ClickUp em abstrações reutilizáveis, eliminando acoplamento dos workflows e permitindo evolução independente da integração.
...
- Todos os workflows automaticamente usam nova versão
...
- Todos os workflows herdam mudança
```

### ✅ Testabilidade:
```
ANTES:
- Testar padrão de comentário em 5 workflows

DEPOIS:
- Testar uma vez na abstração
- Confiança que todos os workflows usam padrão testado
```
...
1. **Criar abstrações** - Implementar em `docs/reference/clickup-mcp-wrappers.md`
2. **Refatorar workflows** - Remover acoplamento de cada workflow
3. **Atualizar documentação** - Remover exemplos de implementação
4. **Validar** - Testar que tudo funciona
5. **Documentar padrões** - Colocar em `docs/reference/`

---

## 📚 Relacionado

- [Padrões de Formatação ClickUp](../../.agents/skills/common/prompts/clickup-patterns.md)
