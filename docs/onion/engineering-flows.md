...
- **Sessions estruturadas** em `docs/sessions/<feature-slug>/`
...
-  Cria pasta `docs/sessions/auth-oauth2/`
...
```bash
/engineer-work docs/sessions/auth-oauth2/
```
...
- Sessão: docs/sessions/auth-oauth2/
...
```bash
/engineer-work docs/sessions/bug-dashboard-login/
```
...
- _Específico do ClickUp:_ comentários usam formatação visual Unicode (`━━━`, `∟`, `▶`, `◆`, `✅`) com timestamp + status obrigatórios — ver `docs/reference/task-manager/adapters/clickup.md`
...
**Próximo**: [Task Manager Abstraction →](../knowledge-base/concepts/task-manager-abstraction.md) · Adapters por provedor em `docs/reference/task-manager/adapters/` (`jira.md`, `clickup.md`, `asana.md`, `linear.md`)
