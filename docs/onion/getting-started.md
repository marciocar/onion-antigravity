...
Bem-vindo ao Sistema Onion (Google Antigravity)! Este guia vai te ajudar a começar rapidamente com os workflows em `.agents/workflows/` e a integração com gerenciadores de tarefas através do **Task Manager Abstraction**.

## 📊 Visão Geral

| Componente | Quantidade | Descrição |
|------------|------------|-----------|
| Workflows | 78 | `.agents/workflows/` (flat + prefixo de categoria) |
| Personas / subagents | — | `.agents/AGENTS.md` (consolidados de 49 agentes especializados) |
| Skills | 2 | `.agents/skills/` (`onion`, `onion-validation`) |
| Rules | 4 | `.agents/rules/` (always-on) |
| Knowledge Bases | — | `docs/knowledge-base/` (consumo por IA) |

## 📋 Checklist de Setup

### **✅ Pré-requisitos**
- [ ] Google Antigravity instalado e configurado (config global em `~/.gemini/`)
- [ ] Git inicializado no projeto
- [ ] Pasta `.agents/` presente no projeto
...
```bash
/engineer-work docs/sessions/about-page/
```
...
/engineer-work docs/sessions/feature-x/ # → Desenvolvimento
...
pwd  # Deve estar na raiz do projeto com .agents/
...
5. **Adapters por provedor** - Detalhes específicos de cada um em `docs/reference/task-manager/adapters/` (`jira.md`, `clickup.md`, `asana.md`, `linear.md`)
...
### **Problema**: Workflows `.agents/` não funcionam
**Sintomas**: Workflows não são reconhecidos
...
pwd  # Deve estar na raiz com .agents/

# 3. Orquestrador / navegação do framework
@onion "workflows não funcionam"
```
