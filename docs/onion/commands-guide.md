...
> **Última atualização**: 2026-06-03 | **Total**: 78 workflows em `.agents/workflows/`

Este guia documenta os workflows disponíveis no Sistema Onion (Google Antigravity), organizados por categoria e função. Workflows são saved prompts invocáveis via `/` em `.agents/workflows/`.

## 📊 Resumo por categoria

| Categoria | Workflows | Descrição |
|-----------|-----------|-----------|
| `engineer-*` | Fluxos de desenvolvimento (faseado retomável) |
| `product-*` | Gestão de produto e descoberta |
| `git-*` | Operações Git (GitFlow achatado) |
| `docs-*` | Documentação |
| `meta-*` | Meta-workflows (criadores de artefatos) |
| `validate-*` · `test-*` · `quick-*` · `development-*` | Validações, testes e ações rápidas |
| **Total** | **78 workflows** |

## 📋 Índice por Categoria

- [🔧 Workflows de Engenharia](#-comandos-de-engenharia)
- [📋 Workflows de Produto](#-comandos-de-produto)
- [📚 Workflows de Documentação](#-comandos-de-documentacao)
- [⚙️ Meta Workflows](#️-meta-comandos)
- [🌲 Workflows Git](#-comandos-git)
- [🌟 Workflows Globais](#-comandos-globais)

---

## 🎯 Como Usar os Workflows

### ⚡ **Workflows do Antigravity vs Terminal**

**TODOS** os workflows deste guia são **saved prompts invocáveis via `/`** no agente do **Google Antigravity** (não são comandos de terminal):

```markdown
# ✅ CORRETO - No agente do Antigravity:
...
### 🚀 **Convenção de naming (flat + prefixo)**
Workflows seguem o padrão `<categoria>-<comando>.md`, invocados `/<categoria>-<comando>`:
- Nomes em kebab-case, prefixo de categoria obrigatório (resolve colisões de `start`/`finish`/`help`)
- Comandos antes aninhados (`/git/feature/start`) viram `<cat>-<sub>-<cmd>` (`/git-feature-start`)
- Personas/subagents documentados em `.agents/AGENTS.md`; skills em `.agents/skills/`
...
2. Cria pasta `docs/sessions/<feature_slug>`
