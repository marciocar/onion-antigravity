---
description: Gerar arquitetura de contexto técnico multi-arquivo em docs/technical-context/ a partir do codebase e materiais relacionados.
---
...
Entrada do usuário: o texto após o comando — paths do codebase, repositórios, configs e referências técnicas. Parâmetro `sources` (obrigatório).

---

## 🎯 Objetivo

Gerar a arquitetura de contexto técnico na pasta canônica `docs/technical-context/`, organizada em 4 camadas (Núcleo / AI-Context / Domínio / Workflow). Use o template de contexto técnico disponível em `.agents/skills/` como base.
...
> Se a entrada estiver vazia, solicite as fontes ao usuário antes de prosseguir.
...
> **Projetos sem manifesto/build (doc-as-code, config-as-code, IaC, frameworks interpretados em runtime).** Quando não houver `package.json`/`requirements.txt`/CI clássico (ex.: um framework em Markdown+YAML como o próprio Onion), NÃO force a descoberta de "stack de build". Em vez disso, trate como stack: o formato dos artefatos (Markdown/YAML/JSON), o runtime que os interpreta, as convenções de diretório e os contratos entre componentes. Registre "sem build/manifesto" como característica explícita, não como lacuna.
...
Fontes podem se contradizer (ex.: `.agents/AGENTS.md` afirma identidade atual mas `CONTRIBUTING.md`/docs antigos descrevem visão abandonada). Aplique esta **ordem de precedência** (da mais forte para a mais fraca):

1. Código e configuração reais (o que executa)
2. `.agents/AGENTS.md` e regras em `.agents/rules/` (regras vigentes do projeto)
...
Quando houver conflito relevante, **registre-o explicitamente** (no ADR ou charter) em vez de propagar a contradição silenciosamente, e sinalize a fonte desatualizada como follow-up.
...
> **Modo não-interativo (infer-from-evidence).** Quando executado sem usuário disponível (ex.: por um agente, em piloto ou automação), NÃO bloqueie nas perguntas: infira as respostas a partir da evidência do repo, **marque cada inferência** com `[INFERIDO]` e liste as suposições em uma seção "Pendências de validação" no `index.md`. Itens sem evidência viram `[TO BE COMPLETED]` em vez de invenção.

### Fase 3 — Geração

Gere os arquivos em `docs/technical-context/` seguindo a estrutura abaixo. Crie apenas os arquivos relevantes ao projeto.

> **Convenção de nomes (esta seção tem precedência sobre o template-base).** Use **kebab-case minúsculo** para todos os arquivos (`codebase-guide.md`, `project-charter.md`), exatamente como na estrutura abaixo. Se o template-base sugerir nomes em UPPERCASE, **ignore** — a estrutura deste workflow é a autoritativa.
...
- **Template-base**: template de contexto técnico em `.agents/skills/`
