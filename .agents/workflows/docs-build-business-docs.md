---
description: Gerar arquitetura de contexto de negócio multi-arquivo em docs/business-context/ a partir de fontes do produto.
---
...
Entrada do usuário: o texto após o comando — links, paths ou descrições das fontes (README, landing pages, repositórios, materiais de marketing, feedback). Parâmetro `sources` (obrigatório).

---

## 🎯 Objetivo

Gerar a arquitetura de contexto de negócio na pasta canônica `docs/business-context/`, organizada em 4 camadas (Cliente / Produto / Mercado / Operacional). Use o template de contexto de negócio disponível em `.agents/skills/` como base.
...
2. `.agents/AGENTS.md` e docs marcadas como atuais
...
Registre conflitos relevantes explicitamente e sinalize a fonte desatualizada como follow-up, em vez de propagar a contradição.
...
> **Modo não-interativo (infer-from-evidence).** Sem usuário disponível (agente/piloto/automação), não bloqueie: infira das fontes, marque cada inferência com `[INFERIDO]` e liste as suposições numa seção "Pendências de validação" no `index.md`. Sem evidência → `[TO BE COMPLETED]`, nunca invenção.
...
- **Template-base**: template de contexto de negócio em `.agents/skills/`
